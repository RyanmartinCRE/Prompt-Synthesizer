...
with st.form("prompt_form"):
    ...
    goal = st.text_area("💡 What do you want the AI to do?", value=st.session_state.get("goal_input", prefill.get("goal", "")), key="goal_input")
    col1, col2 = st.columns(2)
    with col1:
        tone = st.selectbox("🎭 Tone or vibe", valid_tones, index=valid_tones.index(st.session_state.get("tone_input", prefill.get("tone", "Clear and helpful"))), key="tone_input")
    with col2:
        output_type = st.selectbox("🧾 Output format", ["Text", "Conversation", "Image Prompt", "Markdown", "Bullet List", "JSON"],
                                   index=["Text", "Conversation", "Image Prompt", "Markdown", "Bullet List", "JSON"].index(st.session_state.get("output_type_input", prefill.get("output_type", "Text"))),
                                   key="output_type_input")
    audience = st.text_input("👥 Who's it for? (Optional)", value=st.session_state.get("audience_input", prefill.get("audience", "")), key="audience_input")
    save_txt = st.checkbox("📂 Save this to a .txt file?", key="save_txt_input")
    depth = st.slider("🧬 Prompt Inception Depth", 1, 5, st.session_state.get("depth_input", 1), help="How many layers deep should we go?", key="depth_input")
